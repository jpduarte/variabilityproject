...
#find SS with the current at given vgi wrt current*level
  ...
  return SS
######################
root_dir = '/users/jpduarte/research/variabilityproject/netlist_modelcards/mchspicereduced'
...
montecarloresultsall = open('montecarloresultsallfilemchspicereduced', 'w')
stringtoprint = 'Ioff'+' '+'Ion'+' '+'Lg'+' '+'Weff'+' '+'Cins'+' '+'Ach'+' '+'Nfin'+' '+'vd'+' '+'Vth'+' gmax'+' SS'+' filename'+'\n'
montecarloresultsall.write(stringtoprint)

montecarloresultsallvdlin = open('montecarloresultsallfilevdlinmchspicereduced', 'w')
montecarloresultsallvdlin.write(stringtoprint)
montecarloresultsallvdmed = open('montecarloresultsallfilevdmedmchspicereduced', 'w')
montecarloresultsallvdmed.write(stringtoprint)
montecarloresultsallvdsat = open('montecarloresultsallfilevdsatmchspicereduced', 'w')
...
for namefile in data_files[0][1]:
  ...
  if ((flagfiledata > 0)):
    indexWeff = namefile.find("Weff") 
    indexLg = namefile.find("Lg") 
    indexCins = namefile.find("Cins") 
    indexAch = namefile.find("Ach") 
    indexNfin = namefile.find("Nfin") 
    indexvd = namefile.find("vd") 

    Weff =  namefile[indexWeff+4:indexLg]
    Lg =  namefile[indexLg+2:indexCins]
    Cins =  namefile[indexCins+4:indexAch]
    Ach =  namefile[indexAch+3:indexNfin]
    ...
    if len(header)>0:
      ...
      stringtoprint = str(Ioff)+' '+str(Ion)+' '+Lg+' '+Weff+' '+Cins+' '+Ach+' '+Nfin+' '+vd+' '+str(Vth)+' '+str(gmax)+' '+str(SS)+' '+ namefile[indexWeff:indexvd]+'\n' 
      montecarloresultsall.write(stringtoprint)
