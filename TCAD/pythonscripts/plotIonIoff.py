...
lpindexplot=0
for L in Lg:
    if lgindex < 1010:#lgindex > 13 and 
        for vg in vg_array:
            for vd in vd_array:
	            for vb in vb_array:
	                print vd
...
################################################################plot  
pylab.scatter(Ion,Ioff,s=80, facecolors='none', edgecolors='b') 
pylab.title("Vd SAT")    
ax = pylab.gca()
pylab.xlabel("Ion (A)", fontsize=18)
pylab.ylabel("Ioff (A)", fontsize=18)
...
pylab.savefig('IonIoffvsatv6.png', dpi=300, bbox_inches='tight')
pylab.show()
