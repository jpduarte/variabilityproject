...
foldername = './v7/' 
nameproject = 'finfet_7Lg'               

#dimensions                
Lg_array    = 0.001*np.array([20 ,50 ,100 ,250, 500 ,1000])#
Wt_array    = 0.0076*np.array([1])
WbR_array   = 0.0076/2*np.array([1])
WbL_array   = 0.0076/2*np.array([1])
Hfin_array  = 0.042*np.array([1])
tox_array   = 0.0008*np.array([1])
        ...
        for vg in vg_array:
            for vd in vd_array:
	            for vb in vb_array:
	                ...
	                #filenameaux = foldername + nameproject + 'Wt'+str(Wt[lgindex])+'Lg' + str(L) + 'Hfin'+str(Hfin[lgindex])+'tox'+str(tox[lgindex])+'WbR'+str(WbR[lgindex])+'WbL'+str(WbL[lgindex])+ 'Nfin' + str(Doping_FIN[lgindex])+'vd' + vd +'DATAREADY'
	                if lgindex > 651:
	                    filenameaux = foldername + nameproject + 'Wt'+str(Wt[lgindex])+'Lg' + str(L) + 'Hfin'+str(Hfin[lgindex])+'tox'+str(tox[lgindex])+'WbR'+str(WbR[lgindex])+'WbL'+str(WbL[lgindex])+ 'Nfin' + str(Doping_FIN[lgindex])+'vd' + vd +'DATAREADY'+'DATAREADY'
...
#pylab.savefig('IdsVgsallv6linscale.png', dpi=300, bbox_inches='tight')
pylab.show()
