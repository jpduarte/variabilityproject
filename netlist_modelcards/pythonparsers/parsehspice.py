...
def parsehspicev3(wheretosimpath,filenameoutput,vds,Lg, Weff, Cins, Ach, Doping_FIN ):
  filepath =wheretosimpath
  ...
  for line in outputfiletoread:
    ...
    if (state==0):
      ...
      if line.find("x1.d")>-1:
        filenameaux = filenameoutput + 'Weff'+Weff+'Lg'+Lg+'Cins'+Cins+'Ach'+Ach+'Nfin'+Doping_FIN +'vd'+str(vds[countruns]) + 'DATAREADY.out'
        countruns+=1
        #print filenameaux 
        hspicefile = open(filepath+filenameaux, 'w') 
        hspicefile.write('gateOuterVoltage drainTotalCurrent\n') 
